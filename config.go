...
func loadConfig() *ConfigST {
	...
	results, err := db.Query("select id, name, ip, onDemand, protocol, port, username, password from devices")
	if err != nil {
	...
	for results.Next() {
		...
		stream.URL = (device.protocol + "://" + device.username + ":" + device.password + "@" + device.ip + ":" + device.port)
		stream.OnDemand = true
	
		streams[device.name] = stream
